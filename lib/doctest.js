...
function processLine(
  ...
  if (line.slice (0, prefix.length) === prefix) {
    ...
    if (accum.state === CLOSED) {
      ...
      $test = accum.tests[accum.tests.length - 1];
      $test[accum.state = OUTPUT].values.push ({
...
function commonjsEval(options, source, path) {
  ...
  return run (options, queue, options);
}
        ...
        if (false) {
            ...
            if (expected) {
              if (errored) {
                ...
                actualRepr = io['!'] && expected.value.message === '' ?
                             '! ' + output.value.name :
                             showError (output.value);
              } else {
                pass = !io['!'] && Z.equals (output.value, expected.value);
                actualRepr = show (output.value);
              }
              expectedRepr = (io['!'] ? showError : show) (expected.value);
            } else {
              pass = false;
              expectedRepr = 'no output';
              actualRepr = (errored ? showError : show) (output.value);
            }
          ...
          expecteds.slice (outputs.length).forEach (function(expected) {
            accum.results.push ([
              false,
              'no output',
              (io['!'] ? showError : show) (expected.value),
              io[':']
            ]);
          });
        }
