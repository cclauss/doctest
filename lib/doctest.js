...
function evaluate(options, source, path) {
  return options.module === 'commonjs' ?
    ...
    functionEval (options, source, options.logFunction);
}
  ...
  if (line.slice (0, prefix.length) === prefix) {
    ...
    if (accum.state === CLOSED) {
      ...
      $test[OUTPUT].values[$test[OUTPUT].values.length - 1].value += '\n'
                                                                  + value;
      appendToOutput ($test);
    } else if ((match = MATCH_LOG.exec (trimmedLine)) != null) {
      accum.state = LOG;
...
function functionEval(options, source, logFunction) {
  ...
  //  in _this_ context.
  //
  //  The `evaluate` function takes one or two arguments, `__doctest` is
  //  the doctest runtime broker, and optionally it takes a logger function.
  var evaluate = logFunction == null ?
                 Function ('__doctest', source) :
                 Function ('__doctest', logFunction, source);
  ...
  function log(channel) {
    return function(value) {
      logMediator.emit (channel, value);
    };
  }

  evaluate ({
    enqueue: function(io) { queue.push (io); },
...
function run(options, queue, logMediator) {
